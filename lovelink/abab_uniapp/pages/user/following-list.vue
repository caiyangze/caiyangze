<template>
  <view class="following-list-container">
    ...
    <view v-if="followingList.length === 0 && !loading" class="empty-state">
      ...
      <text class="empty-desc">快去关注感兴趣的人吧</text>
    </view>
    ...
    <view v-else class="following-list">
      <view
        ...
        :key="following.followId || index"
        class="following-item"
        @click="goToUserDetail(following.user)"
      >
        <!-- 用户头像 -->
        <view class="avatar-container">
          <image
            :src="following.user?.avatarUrl || '/static/message/default-avatar.png'"
            class="user-avatar"
            mode="aspectFill"
          ></image>
        </view>
        ...
        <view class="user-info">
          <text class="user-name">{{ following.user?.nickname || '用户' + following.user?.userId }}</text>
          <text class="user-desc">{{ getGenderText(following.user?.gender) }}</text>
        </view>

        <!-- 关注按钮 -->
        <view class="follow-btn-container">
          <button
            :class="getFollowButtonClass(following.isFollow)"
            @click.stop="handleFollow(following, index)"
            :disabled="following.loading"
          >
            {{ getFollowButtonText(following.isFollow) }}
          </button>
        </view>
      </view>
    </view>
...
<script>
import { getFollowingList, toggleUserFollow } from '@/api/user/follow.js'
  ...
  methods: {
    ...
    async handleFollow(following, index) {
      try {
        ...
        const result = await toggleUserFollow(following.user.userId)
        
        if (result.code === 200) {
          // 更新关注状态
          if (following.isFollow === 1) {
            this.followingList[index].isFollow = 0 // 已关注 -> 未关注
          } else if (following.isFollow === 2) {
            this.followingList[index].isFollow = 0 // 相互关注 -> 未关注
          }
